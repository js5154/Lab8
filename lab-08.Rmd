...
weather_noNA %>%
  ...
  summarise(n = n()) %>%
  group_by(Location) %>%
  mutate(rainpercent = n/sum(n)) %>%
  filter(RainToday == "Yes") %>%
  arrange(desc(rainpercent))
...
```{r}
weather_Portland2 <- weather_Portland %>%
  mutate(RainTomorrow = as.numeric(RainTomorrow))
linear_reg() %>%
  set_engine("glm") %>%
  fit(RainTomorrow ~ RainToday, data = weather_Portland2, family = "binomial") %>%
  tidy()
```

2. For each point in our dataset, what are the fitted probabilities that tomorrow it's going to rain? 
.25 if Raintoday was No, .55 if Raintoday was yes 
